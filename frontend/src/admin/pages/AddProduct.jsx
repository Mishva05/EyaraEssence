...
export default function AddProduct() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    stock: 10,
    colorsInput: 'Cream, Blush Pink, Sage Green', // Comma separated default
    featured: false,
    bestseller: false,
    imageURL: 'https://images.unsplash.com/photo-1606744824163-985d376605aa?w=600&auto=format&fit=crop&q=80' // default placeholder
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiService.addAdminProduct(payload);
      showToast(`Product "${formData.name}" added to inventory!`, "success");
      navigate('/admin/products');
    } catch (err) {
      console.error(err);
      showToast("Failed to add product to catalog.", "error");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="space-y-4">
        ...
        <div className="border-b border-brand-beige/50 pb-5">
          <h1 className="text-2xl font-serif font-bold text-brand-charcoal">Add New Product</h1>
          <p className="text-brand-brown text-xs">Fill in details to list a new handcrafted creation in the boutique.</p>
        </div>
      </div>
      ...
      {submitting ? (
        <div className="py-20 flex flex-col items-center justify-center">
          <Spinner size="large" />
          <p className="text-brand-brown text-sm mt-3 font-semibold font-sans">Uploading product profile to inventory...</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* Main Info Fields (Left Col) */}
